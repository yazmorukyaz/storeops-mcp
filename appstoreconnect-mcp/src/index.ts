...
server.registerTool(
  ...
  async ({ app_info_id, locale, limit, response_format }) => {
    ...
    if (locale) query["filter[locale]"] = locale;
...
server.registerTool(
  ...
  async ({ app_id, territory, limit, response_format }) => {
    const query: Record<string, QueryValue> = { limit };
    if (territory) query["filter[territory]"] = territory;
...
async function patchResource(type: string, id: string, attributes: Record<string, unknown>) {
...
async function getLocalizationVisuals(localizationId: string, limit: number) {
  ...
  const screenshots = await Promise.all(screenshotSetItems.map((set) =>
    safeAppStoreConnectRequest("GET", `/appScreenshotSets/${encodeURIComponent(set.id)}/appScreenshots`, { limit })
  ));
  const previews = await Promise.all(previewSetItems.map((set) =>
    safeAppStoreConnectRequest("GET", `/appPreviewSets/${encodeURIComponent(set.id)}/appPreviews`, { limit })
  ));
...
async function auditStoreMarketing(appId: string, versionLimit: number) {
  ...
  const appInfoLocalizations = await Promise.all(appInfoItems.map((item) =>
    safeAppStoreConnectRequest("GET", `/appInfos/${encodeURIComponent(item.id)}/appInfoLocalizations`, { limit: 200 })
  ));
  const versionLocalizations = await Promise.all(versionItems.map((version) =>
    safeAppStoreConnectRequest("GET", `/appStoreVersions/${encodeURIComponent(version.id)}/appStoreVersionLocalizations`, {
      limit: 200,
      include: "appScreenshotSets,appPreviewSets,searchKeywords"
    })
  ));
  const subscriptionGroupItems = subscriptionGroups.ok ? getResources(subscriptionGroups.response) : [];
  const subscriptionsByGroup = await Promise.all(subscriptionGroupItems.map((group) =>
    safeAppStoreConnectRequest("GET", `/subscriptionGroups/${encodeURIComponent(group.id)}/subscriptions`, { limit: 200 })
  ));
  ...
  return {
    ...
    failures: Object.fromEntries(
      Object.entries({ app, appInfos, versions, iaps, subscriptionGroups, customProductPages, promotedPurchases, reviews, analyticsRequests })
...
type SafeAppStoreResponse =
  | { ok: true; response: { status: number; headers: Record<string, string>; data: unknown } }
  | { ok: false; error: string };

async function safeAppStoreConnectRequest(method: HttpMethod, path: string, query?: Record<string, QueryValue>, body?: unknown): Promise<SafeAppStoreResponse> {
  try {
...
function countSafeResources(entry: SafeAppStoreResponse) {
  return entry.ok ? getResources(entry.response).length : 0;
}

function summarizeSafeResponse(entry: SafeAppStoreResponse) {
...
function buildMarketingActions(input: {
  versionLocalizationCount: number;
  missingCommonLocales: string[];
  iapCount: number;
  subscriptionCount: number;
  customProductPageCount: number;
  promotedPurchaseCount: number;
  reviewCount: number;
}) {
  const actions = [];
