...
async function auditPaywallCatalog(projectId: string, customerLimit: number) {
  ...
  const issues = [];
  if (productItems.length === 0) issues.push("No products were returned. Add or import App Store / Google Play / Stripe products before paywall optimization.");
  if (entitlementItems.length === 0) issues.push("No entitlements were returned. Entitlements are the access layer that turns purchases into product value.");
  if (offeringItems.length === 0) issues.push("No offerings were returned. Offerings are required for remotely controlled paywall product mixes.");
  if (packages.ok && packageItems.length === 0) issues.push("No packages were returned from the direct package endpoint. Packages connect offerings to concrete products.");
  if (paywallItems.length === 0) issues.push("No paywalls were returned. Add RevenueCat Paywalls to remotely change conversion UI without app releases.");
  ...
  return {
    ...
    failures: Object.fromEntries(
      Object.entries({ apps, products, packages, entitlements, offerings, customers, paywalls, metrics, webhooks })
...
type RevenueCatResponse = { status: number; headers: Record<string, string>; data: unknown };
type SafeRevenueCatResponse =
  | { ok: true; response: RevenueCatResponse }
  | { ok: false; error: string };

async function safeRevenueCatRequest(method: HttpMethod, path: string, query?: Record<string, QueryValue>, body?: unknown, useV1 = false): Promise<SafeRevenueCatResponse> {
  try {
...
function safeItems(entry: SafeRevenueCatResponse) {
  return entry.ok ? getItems(entry.response) : [];
}

function safeCount(entry: SafeRevenueCatResponse | undefined) {
